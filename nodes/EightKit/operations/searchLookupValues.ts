...
export async function executeSearchLookupValues(
  ...
  try {
    ...
    if (!searchValue?.trim()) {
      throw new NodeOperationError(this.getNode(), 'Search value is required', {
        itemIndex: index,
      });
    }
    ...
    switch (searchType) {
      ...
      case 'search':
        ...
        break;
      default:
        throw new NodeOperationError(this.getNode(), `Invalid search type: ${searchType}`, {
          itemIndex: index,
        });
    }
    ...
    if (!response?.success) {
      throw new NodeOperationError(
        this.getNode(),
        response?.error || 'Failed to search lookup values',
        { itemIndex: index }
      );
    }
